...
func type_out() -> void:
	...
	var princi = get_node("/root/VarPrinci")
	var comportement = data.last_comportement
	var friends = data.last_friends
	var full_random_index = randi_range(0, 3)
	var partial_false_index = randi_range(2, 3)
	var partial_true_index = randi_range(0, 1)
	var randomSentence
	if data.end == true:
		randomSentence = ""
	elif comportement == "mechant" && friends == 1:
		randomSentence = data.get_random_sentence(3)
	elif comportement == "mechant" && friends == 2:
		randomSentence = data.get_random_sentence(2)
	elif comportement == "mechant":
		randomSentence = data.get_random_sentence(partial_false_index)
	elif comportement == "gentil":
		randomSentence = data.get_random_sentence(partial_true_index)
	else:
		randomSentence = data.get_random_sentence(full_random_index)
	data.set_last_sentence(randomSentence)
	var coloredSentence = "[color=yellow]" + randomSentence + "[/color]"
	if princi.count == 2:
		dialogue_line.text = "[color=yellow]" + dialogue_line.text + "[/color]"
	text = dialogue_line.text + "\n" + coloredSentence
